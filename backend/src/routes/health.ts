 ...
 * to begin with — the actual mitigation for Render cold starts, since
 * nothing running *inside* this process can prevent Render from
 * suspending it when idle. Deliberately separate from /api/warmup: this
 * route never triggers warm-up work, so a keep-alive pinger can't
 * accidentally cause repeated, pointless re-warming.
 */
