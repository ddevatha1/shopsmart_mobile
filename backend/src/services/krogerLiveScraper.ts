...
import { markStoreReady, markStoreWarming, logSearchUsedColdPath, type StoreKey } from './storeReadiness.ts';
...
// One shared token backs both banners (see this file's header comment) —
// readiness is reported for both keys together whenever it's (re)established.
function markTokenReady(): void {
  markStoreReady('kroger');
  markStoreReady('harristeeter');
}
...
async function getToken(): Promise<string> {
  ...
  markStoreWarming('kroger');
  markStoreWarming('harristeeter');
...
export interface KrogerBanner {
  ...
  idSlug: string;
  /** This banner's key in the storeReadiness registry. */
  readinessKey: StoreKey;
}

const KROGER_BANNER: KrogerBanner = { storeName: 'Kroger', chain: 'KROGER', idSlug: 'kroger', readinessKey: 'kroger' };
const HARRIS_TEETER_BANNER: KrogerBanner = { storeName: 'Harris Teeter', chain: 'HART', idSlug: 'harris-teeter', readinessKey: 'harristeeter' };
...
async function searchKrogerBanner(
  ...
  // Cheap enough (an OAuth token round-trip) to bootstrap inline if warm-up
  // hasn't already done it — unlike Trader Joe's, never worth skipping the
  // store over. Logged purely for cold-start observability.
  const hadWarmToken = tokenCache !== null && Date.now() < tokenCache.expiresAt;
  const tokenStart = Date.now();
  const token = await getToken();
  if (!hadWarmToken) logSearchUsedColdPath(banner.readinessKey, Date.now() - tokenStart);
