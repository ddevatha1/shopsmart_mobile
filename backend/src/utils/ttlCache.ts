...
export class TtlCache<T> {
  ...
  // Every instance registers itself here so the internal-dashboard route
  // can report one aggregate hit rate across every store integration's
  // cache (krogerLocator, traderJoesLocator, etc.) without each call site
  // needing to expose its own instance separately. A `WeakRef` would be
  // "more correct" for a cache that's expected to be garbage-collected —
  // in practice every TtlCache in this codebase is a module-level
  // singleton that lives for the whole process, so a plain array never
  // actually accumulates anything that wasn't going to stay alive anyway.
  private static instances: TtlCache<unknown>[] = [];

  constructor(ttlMs: number) {
    this.ttlMs = ttlMs;
    TtlCache.instances.push(this as TtlCache<unknown>);
  }
  ...
  /** Aggregated across every TtlCache instance in this process — "cache
   * effectiveness" for the internal dashboard, not a per-lookup analytics
   * event (that would be far too high a volume for what's meant to be a
   * cheap, always-on counter). Resets to 0/0 on every process restart,
   * same as everything else in this stateless backend — see
   * internalDashboard.ts's own note on what "since last restart" means. */
  static getAggregateStats(): { hits: number; misses: number; hitRate: number | null } {
    let hits = 0;
    let misses = 0;
    for (const cache of TtlCache.instances) {
      hits += cache.hits;
      misses += cache.misses;
    }
