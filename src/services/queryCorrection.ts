 ...
 * call, no ML model. This app's backend already has its own copy (see
 * backend/src/services/queryCorrection.ts, same "no shared runtime between
 * this app and its backend" reasoning documented throughout — comparisonService,
 * groceryTaxonomy); this is a second, independent copy for the Smart
 * Shopping Planner's client-side ambiguity resolution (see
 * plannerAmbiguityService.ts), which needs typo correction to run
 * instantly, before any network call, the same way the web app's copy does.
 */
